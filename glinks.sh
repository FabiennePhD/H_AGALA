...
# this will work with any topology file (input it based on topol.top as per the grep commands below)  that has any chain length and is protonated, just bash glinks.sh - the final output is topol4.top

# ignore the SED way -its there in case I need it for future stuff
...
$0 ~ /funct/ {
    if (flag==1){
         for (i=1;i<narrA-4;i+=4) {printf "%5s %5s %5s %5s\n",arrA[i],arrA[i+1],arrA[i+2], arrA[i+3]};      flag=0;
	}
}' > topol3.top
...
for ((i=0; i<($lO4); i++)); do
    ...
    strvar_g+="${O4[i]}"$':'"${C1[i]}"$':'"${C2[i+1]}"$':'"${H2[i+1]}"$':'"9"$':'
    strvar_h+="${O4[i]}"$':'"${C1[i+1]}"$':'"${C2[i+1]}"$':'"${O2[i+1]}"$':'"9"$':'
    strvar_i+="${O4[i]}"$':'"${C1[i+1]}"$':'"${C2[i+1]}"$':'"${C3[i]}"$':'"9"$':'

done

string_1="$strvar_a$strvar_b$strvar_c$strvar_d$strvar_e$strvar_f$strvar_g$strvar_h$strvar_i"
